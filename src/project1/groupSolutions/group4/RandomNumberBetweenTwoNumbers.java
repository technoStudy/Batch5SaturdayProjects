...
public class RandomNumberBetweenTwoNumbers {
    ...
    public static void main(String[] args) {
        ...
        int swap;     //Challenge: it will swap first and second number if needed
        if (min > max) {
            swap = max;
            max = min;
            min = swap;
        }
        int randomNumber = (int) (Math.random() * (max - min +1)) + min;        //NOTE: max is inclusive
        System.out.println("Random number between " + min + " and " + max + " (inclusive) is " + randomNumber);
    }
}
